 ...
 * Additionally it can be used to keep tallies across several rounds of play. Side bets are not
 * supported.
 *
...
public class Game {
  ...
  /**
   * Prepares this instance to play a round&mdash;i.e. returns the game state
   * to {@link State#COME_OUT}, clears the established point, and clears the list of rolls.
   * This method <strong>must</strong> be invoked prior to {@link #play()}, even
   * immediately after creating and initializing the instance via {@link #Game(Random)}.
   */
  public void reset() {
   ...
   * invoked on an instance already in a terminal state{@link State#WIN} or {@link State#LOSS}, no
   * exception is thrown, but the state of this <code>Game</code> instance doesnt change.
  ...
  /**
   * Returns current state of game. Tis will be the {@link State#COME_OUT} state after
   * {@link #reset()} is invoked, and a terminal state ({@link State#WIN} or
   * {@link State#LOSS} after {@link #play()} is invoked.
   * @return
   */
